...
public class FileDataSource implements HeatMapDataSource {
	...
	private void LoadPointsFromFile(String source, int longitudeIndex, int latitudeIndex, int weightIndex) {
		...
		for (String line : lines) {
			item = line.split(",");
			_pointList.add(new PointLatLng(Double.parseDouble(item[longitudeIndex]), Double.parseDouble(item[latitudeIndex]), Double
					.parseDouble(item[weightIndex])));
		}
	}
	...
	public PointLatLng[] GetList(DataPoint tlb, DataPoint lrb, int zoom, Projections _projection) {
		...
		for (PointLatLng point : _pointList) {
			if ((point.getLatitude() <= ptlb.getLatitude()) && (point.getLongitude() >= ptlb.getLongitude())
					&& (point.getLatitude() >= plrb.getLatitude()) && (point.getLongitude() <= plrb.getLongitude())) {
				llList.add(point);
			}
		}
